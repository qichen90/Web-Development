...
engine = create_engine('postgres://cmvbrzkcxbsnfi:NQoPLVRsGtnpiy24WE949_5o8P@ec2-54-83-202-64.compute-1.amazonaws.com:5432/d1mv3vs75f2v0q')
Base.metadata.bind = engine

DBSession = sessionmaker(bind=engine)
session = DBSession()

###----------Log in and Log out using Third-party API-----------###
...
def editRestaurant(restaurant_id):
  ...
  if editedRestaurant.user_id != login_session['user_id']:
    return "<script> function info(){alert('You are not authorized to edit this restaurant.');}</script><body onload='info())''>"
  if request.method == 'POST':
...
def deleteRestaurant(restaurant_id):
  ...
  if restaurantToDelete.user_id != login_session['user_id']:
    return "<script> function info(){alert('You are not authorized to delete this restaurant.');}</script><body onload='info()''>"
  if request.method == 'POST':
...
def newMenuItem(restaurant_id):
  ...
  if login_session['user_id'] != restaurant.user_id:
    return "<script> function info(){alert('You are not authorized to add this item.');}</script><body onload='info()''>"
  if request.method == 'POST':
...
def editMenuItem(restaurant_id, menu_id):
  ...
  restaurant = session.query(Restaurant).filter_by(id = restaurant_id).one()
  if login_session['user_id'] != restaurant.user_id:
    return "<script> function info(){alert('You are not authorized to edit this item.');}</script><body onload='info()''>"
  if request.method == 'POST':
    if request.form['name']:
...
def deleteMenuItem(restaurant_id,menu_id):
  ...
  if login_session['user_id'] != restaurant.user_id:
    return "<script> function info(){alert('You are not authorized to delete this items.');}</script><body onload='info()''>"
  if request.method == 'POST':
